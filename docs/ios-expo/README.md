...
| 3 | Screens 🟡 *26 Sep 2026: every offline screen built — all five tabs, onboarding and the discovery cards, profiles (add/edit/file export and import), both chart styles, Panchang lagna chart and insights, chart picture share, Guna Milan PDF, GPS city and geocoder search. Waiting on the TestFlight walk-through. Not built: saved reports (never saved in the Play app, see FEATURE_PARITY 4.16)* | All screens from `FEATURE_PARITY.md`, offline features complete | TestFlight walk-through | 2–4 weeks |
| 4 | Services 🟡 *26 Sep 2026: local notifications done (daily, Muhurat, festival; no push). The rest wait on console setup — `OWNER_CHECKLIST.md`* | Sign-in, cloud backup, AI (PRO), ads, purchases, notifications, widgets | Each service checked on the iPhone | 1–2 weeks |
